...
Load unpaired images from two domains using the `unaligned_dataset` loader.
Train two generators and two discriminators to perform bidirectional translation between the two morphology domains.
Apply adversarial and cycle-consistency constraints to preserve structural content while learning cross-domain style / morphology transfer.
Save checkpoints and intermediate samples during training.
Generate translated / fused morphology images during testing for downstream visualization, aesthetic evaluation, and later 3D modeling.
Dataset format
...
Organize biomimetic images into `trainA`, `trainB`, `testA`, and `testB`.
Train a CycleGAN model for a selected morphology pair.
Inspect saved checkpoints and generated sample images during training.
Run `test.py` on the trained model to export generated fusion morphologies.
Use generated outputs for later quantitative evaluation, visualization, or 3D reconstruction steps described in the manuscript.
